...
1. There are __four__ basic families of geoms that display uncertainty depending on whether the ``x`` values are discrete or continuous, and whether or not you want to display the middle of the interval, or just the extent. 

a. __Discrete x, Range__: ``geom_errorbar()``, ``geom_linerange()``

b. __Discrete x, Range, Center__: ``geom_crossbar()``, ``geom_pointrange()``

c. __Continuous x, Range__: ``geom_ribbon()``

d. __Continuous x, Range, Center__: ``geom_smooth(stat="identity")``

2. These geoms assume that you are interested in the __distribution of y contiditional on x__ and use the aesthetics ``ymin`` and ``ymax`` to determine the range of the ``y`` values. 
...
1. The choice of a weighting variable profoundly affects what we are looking at in the plot and the conclusions that we will draw. 

a. There are __two aesthetics attributes__ that can be used to adjust for weights:

(1) __Size Aesthetic__: adjust for weights for simple geoms like lines and points. 
...
(2) __Weight Aesthetic__: adjust for weights for more complicated grobs which involve some statistica transformation. 

(a) These weights will be passed on to the statistical summary function. 

(b) Weights are supported for every case where it makes sense: smoothers, quantile regressions, boxplots, histograms, and density plots. 

(c) You can NOT see this weighting variable directly, and it does NOT produce a legend, but it will change the results of the statistical summary. 
...
1. There are number of geoms that can be used to display distributions, depending on the __dimensionality__ of the distribution, whether it is __continuous__ or __discrete__, and whether you are interested in the __conditional__ or __joint__ distribution. 


a. __geom_histogram__: for 1d continuous distributions, the most important geom is the __histogram__. 
...
This reveals far more detail than the previous plot
```{r}
ggplot(diamonds, aes(depth))+
...
(1) Important to experiment with binning to find a revealing view. 

(a) Can change the ``binwidth``, specify the number of ``bins``, or specify the exact location of the ``breaks``. 


(b) When publishing figures, include information about important paraeters (like bin width) in the caption. 

(2) Options for ompare the distribution between groups: 

(a) __facet_wrap(~var)__: show small multiples of the histogram

(b) __geom_freqpoly()__: use color and frequency polygon
...
(c) __geom_histogram(position="fill")__: use a "conditional density plot". It uses ``position_fill()`` to stack each bin, scaling it to the same height. 

(1) ``geom_histogram(position="fill")`` is perceptually chanllenging as you need to compare bar heights NOT positions, but you can see the strongest patterns. 
...
b. BOTH the histogram and frequency polygon geom use the SAME underlying statistical transformation: ``stat="bin"``. 
(1) This statistic produces two output variables: ``count`` and ``density``. 

(a) __Count__: by default, count is mapped to y-position, because it's more interpretable. 

(b) __Density__: the density is the count divided by the total count multiplied by the bin width. 

<1> The density is useful when you want to compare the __shape__ of the distributions, NOT the overall size. 

c. __geom_density()__: it's a density estimate. ``geom_density()`` places a little normal distribution at each data point and sums up all the curves. 

(1) It is an alternative to a bin-based visualization. 

(2) It has desirable theoretical properties but is more difficult to relate back to the data. 

(3) Use a density plot when you know that the underlying density is smooth, continuous and unbounded. 

(4) You can use the ``adjust`` parameter to make the density more or less smooth. 
...
(5) The area of each density estimate is standardized to one so that you lose information about the relative size of each group. 

d. The __histogram__, __frequency polygon__, and __density plot__ display a detailed view of the distribution. 


e. __geom_boxplot()__: the box-and-whisker plot shows __five summary statistics__ along with individual "outliers". 

(1) It displays far less information than a histogram, but also takes up much less space. 

(2) You can use boxplot with BOTH categorical and continuous ``x``. 

(a) For __continuous x__, you also need to set the group aesthetic to define how the ``x`` variable is broken up into bins. A useful helper function is ``cut_width()``


Boxplot of discrete ``x`` (clarity) 
```{r}
...
Boxplot of continuous ``x`` (carat) with group aesthetic
```{r}
...
f. __geom_violin()__: the violin plot is a compact version of the density plot. The underlying computation is the same, but the results are displayed in a similar fashion to the boxplot. 


Violin plot of discrete ``x`` (clarity)
```{r}
ggplot(diamonds, aes(clarity, depth))+
  geom_violin()
```
...
Violin plot of continuous ``x`` (carat) with group aesthetic
...
g. __geom_dotplot__: draws one point for each observation, carefully adjust in space to avoid overlaps and show the distribution.

(1) ``geom_dotplot`` is useful for smaller datasets.
...
range(diamonds$carat) #0.20, 5.01


bin01=ggplot(diamonds, aes(carat))+geom_histogram(binwidth=0.1)


bin03=ggplot(diamonds, aes(carat))+geom_histogram(binwidth=0.3)

bin001=ggplot(diamonds, aes(carat))+geom_histogram(binwidth=0.01)


bin005=ggplot(diamonds, aes(carat))+geom_histogram(binwidth=0.05)


ggarrange(bin001, bin005, bin01, bin03, labels=c("binwidth=0.01", "binwidth=0.05", "binwidth=0.1", "binwidth=0.3"), ncol=2, nrow=2)

```

Binwidth of 0.05 works best. 
...
There is a dip around price =$125.


3. How does the distribution of ``price`` vary with ``clarity``?

```{r}
ggplot(diamonds, aes(price, color=clarity, fill=clarity))+
...
range(diamonds$depth) #43, 79
ggplot(diamonds, aes(depth, ..density..))+
  geom_freqpoly(binwidth=1)

ggplot(diamonds, aes(depth, ..count..))+
  geom_density()



```
To make the two plots comparable, we can either map ``y`` to ``...density`` in ``geom_freqpoly()`` or map ``y`` to ``..count`` in ``geom_density()``.



## 3.12 Dealing with Overplotting ##

1. Scatterplot is very important tool for assessing the __relationship between two continuous variables__. 

a. __Overplotting Problem__: when the data is large, points in scatterplot will be often plotted on top of each other, obscuring the true relationship. 

b. __Overplotting Problem Solutions__:

(1) Making the points smaller, or use hollow glyphs: for very small amounts of overplotting, we can make the points smaller, or use hollow glyphs. 
...
(2) Alpha blending (transparency) to make the points transparent: for larger datasets with more overplotting, we can use alpha blending (transparency) to make the points transparent. 

(a) If specify ``alpha`` as a ratio, the denominator gives the number of points that must be overplotted to give a solid color.

(b) Values smaller than ~1/500 are rounded down to zero, giving completely transparent points. 

```{r}
norm+geom_point(alpha=1/3) # Need 3 points to be overplotted to make a solid point

norm+geom_point(alpha=1/5)# Need 5 points to be overplotted to make a solid point

norm+geom_point(alpha=1/10)# Need 10 points to be overplotted to make a solid point
```


!!!! DISCRETENESS????
(3) Randomly jitter the points to alleviate some overlaps with ``geom_jitter``: if there is some discreteness in the data, can randomly jitter the points to alleviate some overlaps with ``geom_jitter()``. 

(4) Think of overplotting as a 2d density estimation problem:

(a) Bin the points and count the number in each bin, then visualize that count (the 2d generalisation of the histogram), ``geom_bin2d``. 

<1> __Problem__: Breaking the plot into many small squares can produce distracting visual artefacts.

2d density estimation with default number of bins
```{r}
...
2d density estimation with 10 number of bins
```{r}
...
(b) Use hexagons instead of small squares to avoid distracting visual artefacts. Use ``geom_hex()`` from the ``hexbin`` package. 

2d hexgons with default number of bins
```{r}
library(hexbin)
...
2d hexgons with 10 number of bins

```{r}
norm+geom_hex(bins=10)
```

(c) Estimate the 2d density with ``stat_density2d()`` and then disply using one of the techniques for showing 3d surfaces in Sect 3.6. 

<1> ``geom_contour()``: contour plot

<2> ``geom_raster()``: colored tiles

<3> ``geom_point(aes(size=density), alpha=1/3)+scale_size_area()``: bubble plot



(5) Add data summaries to help guide the eye to the true shape of the pattern within the data. 

(a) For example, you can add a smooth line showing the center of the data with ``geom_smooth()``. 



## 3.13 Statistical Summaries ##

1. __geom_histogram()__ and __geom_bin2d()__: use a familiar geom, ``geom_bar()`` and ``geom_raster()``, combined with a new statistical transformation, ``stat_bin()`` and ``stat_bin2d()``. 

a. __stat_bin()__ and __stat_bin2d()__: combine the data into bins and count the number of observations in each bin. 

2. __stat_summary_bin()__ and __stat_summary_2d__: compute different summaries other than count. 


Count the number of diamonds in each bin using bar plot
```{r}
...
!!! ERROR MSG!!
Compute the average price in each bin using bar plot
...
!!!! meaning????
Count the number of diamonds in each bin using raster plot
```{r}
...
2. These summary functions are quite constrained but are often useful for a quick first pass at a problem. 
...
Q3 Which transformation comes first, scale or statistics? __Scale__
...
# Problem with this code: the data is NOT explicitly identify in the geom_line layer
ggplot(mpg, aes(displ, hwy)) +

  geom_point() +

  geom_line(grid, color = "blue", size = 1.5) + 

  geom_text(outlier, aes(label = model))
...
Q7 Mapping a variable to an aesthetic happens inside of ``aes()`` but if you set a constant like "blue" inside aes it: __maps blue as a new variable__. 


Q8 The solid line in a QQ plot is a regression line of Y~X: __FALSE__ (A solid line in the q-q plot indicates the point where ``x`` (theoretical or observed values) equals ``y`` (observed or theoretical values)).


Q9 A violin plot is a __density__plot mirrored over its axis. 

Q12 The choice of using a cartogram map is good at treating states equally through visuals but they can __underrepresent__ populus state and __overrepresent__ less populus states. 
