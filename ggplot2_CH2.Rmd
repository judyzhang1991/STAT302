...
```
The value "blue" is scaled to a pinkish color, and a legend is added. When inside ``aes()`` function, it's treated as a variable.  
...
1. What is the problem with the plot created by ``ggplot(mpg, aes(cty, hwy))+geom_point()``? Which of the geoms described above is most effective at remedying the problem?
...
1. ``xlab()`` and ``ylab()``: modify the x- and y-axis labels:
```{r}
...
1. Conventions: plot ``y`` by ``x``; regress ``y`` by ``x``; plot ``y`` over ``x``.

a. If it says make a plot of ``this`` and ``that``, then no clear indication on what should go on x-axis and what should go on y-axis. Use context/domain knowledge to determine which is the response variable (``y``) and which is the explanatory varible (``x``).